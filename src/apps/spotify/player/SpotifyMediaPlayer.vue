<template>
  <Transition name="slide-up">
    <div v-if="show" class="absolute inset-0 z-[150] flex flex-col" style="background: rgb(18,18,18);">
      <!-- Fondo blureado -->
      <div
      ...
      <div class="relative flex-1 overflow-y-auto no-scrollbar">
        ...
        <div class="flex items-center justify-between px-5 pt-6 pb-4">
          ...
          <div class="text-center">
            <p class="text-white/50 text-[10px] uppercase tracking-[0.18em] font-semibold">{{ context }}</p>
            <p class="text-white text-sm font-bold mt-0.5 max-w-[180px] truncate">{{ artistName }}</p>
          </div>

          <button class="w-9 h-9 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-white/60">
        ...
        <div class="px-6 mb-8">
          <div
            ...
            style="background: rgb(40,40,40); box-shadow: rgba(0,0,0,0.6) 0px 32px 64px; transform: scale(0.88);"
          >
        ...
        <div class="px-6 flex items-start justify-between mb-6">
          <div class="flex-1 min-w-0">
            <p class="text-white text-2xl font-black truncate">{{ track.title }}</p>
            <p class="text-base font-medium mt-0.5" style="color: rgb(179,179,179);">{{ track.artist }}</p>
          </div>
          <button class="ml-4 mt-1.5 flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: rgb(179,179,179);">
        ...
        <!-- Barra de progreso -->
        <div class="px-6 mb-5">
          <div class="w-full h-1 rounded-full" style="background: rgba(255,255,255,0.14);">
            <div class="h-full rounded-full" :style="{ width: `${track.progress ?? 0}%`, background: 'white' }" />
          </div>
          <div class="flex justify-between mt-1.5">
            <span class="text-[11px]" style="color: rgb(179,179,179);">0:00</span>
            <span class="text-[11px]" style="color: rgb(179,179,179);">-{{ track.duration }}</span>
          </div>
        </div>
        ...
        <div class="px-5 flex items-center justify-between mb-10">
          ...
          <!-- Play -->
          <button class="w-16 h-16 rounded-full flex items-center justify-center flex-shrink-0" style="background: white;">
        ...
        <section class="px-5 mb-6">
          <div class="rounded-2xl overflow-hidden" style="background: rgb(192,57,43);">
            ...
            <div class="relative overflow-hidden px-4 pb-2" style="max-height: 230px;">
              <p class="font-black leading-snug mb-4" style="font-size: 23px; color: rgba(0,0,0,0.88);">{{ lyrics }}</p>
              <div class="absolute bottom-0 left-0 right-0"
                style="height: 72px; background: linear-gradient(to top, rgb(192,57,43) 0%, transparent 100%);" />
            </div>
            <button class="px-4 pb-4 text-sm font-black" style="color: rgba(0,0,0,0.55);">Ver más</button>
          </div>
        </section>
          ...
          <div class="mx-5 rounded-2xl overflow-hidden" style="background: rgb(40,40,40);">
            ...
            <div class="flex gap-3 px-4 pb-4 overflow-x-auto no-scrollbar">
              ...
              >
                ...
                <p class="absolute bottom-2.5 left-2.5 right-2.5 text-white text-xs font-bold leading-tight">
                  {{ artistName }}
                </p>
              </div>
            </div>
